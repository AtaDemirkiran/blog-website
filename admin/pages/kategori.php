...
<body>
    ...
    //Aktif Degistirme
        ...
        $aktif= $db->prepare("UPDATE `kategori` SET `aktif` = :a  WHERE `id`=:i ");
        ...
        if(@$_GET["is"]=="sil"){
            $sil=$db->prepare("DELETE FROM  `kategori` WHERE `id` = :i ");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <a href="kategori_ekle.php" class="btn btn-primary" style="margin-right: 15px;">Kategori Ekle +</a>
                            <?php
                                    if(@$_GET["i"]=="ekle"){
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <?php
                                            $cek=$db->prepare("SELECT * FROM `kategori` ORDER BY `id` DESC");
                                            $cek->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><img width="50px" height="50px" src="<?= $row["image"] ?>" />
                                                    <a href="kategoriFoto.php?id=<?=$row["id"]?>"
                                            ...
                                            <td class="center">
                                                <?php if($row["aktif"] == 1){ ?>
                                                <a href="kategori.php?is=aktif&id=<?=$row["id"]?>&drm=<?=$row ["aktif"] ?>"
                                            ...
                                            <td>
                                                <a href="kategori_duzenle.php?id=<?=$row["id"]?>"
                                                    class="btn btn-warning btn-xs"
                                                    style="margin-right: 15px;">Düzenle</a>
                                                <a href="kategori.php?is=sil&id=<?=$row["id"]?>"
