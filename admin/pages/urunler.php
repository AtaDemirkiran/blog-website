...
<body>
    ...
    //Aktif Degistirme
        ...
        $aktif= $db->prepare("UPDATE `urunler` SET `aktif` = :a  WHERE `id`=:i ");
        ...
        if(@$_GET["is"]=="sil"){
            $sil=$db->prepare("DELETE FROM  `urunler` WHERE `id` = :i ");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <a href="urunler_ekle.php" class="btn btn-primary" style="margin-right: 15px;">Ürün Ekle +</a>
                            <?php
                                    if(@$_GET["i"]=="ekle"){
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <?php
                                            $cek=$db->prepare("SELECT * FROM `urunler` ORDER BY `id` DESC");
                                            $cek->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><img width="50px" height="50px" src="<?= $row["image"] ?>" />
                                                    <a href="urunlerFoto.php?id=<?=$row["id"]?>"
                                                    class="btn btn-info btn-xs"
                                                    style="margin-right: 15px;">Değiştir</a>
                                                
                                            </td>
                                            <td>
                                                <a href="urundetay.php?id=<?=$row["id"]?>"
                                                    class="btn btn-warning btn-xs"
                                                    style="margin-right: 15px;">Detay Fotoğraf</a>
                                            </td>
                                            <td class="center">
                                                <?php if($row["aktif"] == 1){ ?>
                                                <a href="urunler.php?is=aktif&id=<?=$row["id"]?>&drm=<?=$row ["aktif"] ?>"
                                            ...
                                            <td>
                                                <a href="urunler_duzenle.php?id=<?=$row["id"]?>"
                                                    class="btn btn-warning btn-xs"
                                                    style="margin-right: 15px;">Düzenle</a>
                                                <a href="urunler.php?is=sil&id=<?=$row["id"]?>"
                                                    onclick="return confirm('Silmek İstediğinze Emin Misinz ?')"
