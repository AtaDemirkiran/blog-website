...
<body>
    ...
    //Aktif Degistirme
        ...
        $aktif= $db->prepare("UPDATE `galeri` SET `aktif` = :a  WHERE `id`=:i ");
        ...
        if(@$_GET["is"]=="sil"){
            $sil=$db->prepare("DELETE FROM  `galeri` WHERE `id` = :i ");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <a href="galeri_ekle.php" class="btn btn-primary" style="margin-right: 15px;">Fotoğraf Ekle +</a>
                            <?php
                                    if(@$_GET["i"]=="ekle"){
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <?php
                                            $cek=$db->prepare("SELECT * FROM `galeri` ORDER BY `id` DESC");
                                            $cek->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td class="center">
                                                <?php if($row["aktif"] == 1){ ?>
                                                <a href="galeri.php?is=aktif&id=<?=$row["id"]?>&drm=<?=$row ["aktif"] ?>"
                                            ...
                                            <td>
                                                <a href="galeri_duzenle.php?id=<?=$row["id"]?>"
                                                    class="btn btn-warning btn-xs"
                                                    style="margin-right: 15px;">Düzenle</a>
                                                <a href="galeri.php?is=sil&id=<?=$row["id"]?>"
