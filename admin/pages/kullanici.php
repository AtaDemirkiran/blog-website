...
<body>
    ...
    //Aktif Degistirme
        ...
        $aktif= $db->prepare("UPDATE `kullanici` SET `aktif` = :a  WHERE `id`=:i ");
        $aktif->bindValue(":a",$durum,PDO::PARAM_INT);
        $aktif->bindValue(":i",$id,PDO::PARAM_INT);
            if($aktif->execute()){
                header("Location:kullanici.php?i=ekle");
            }else{
                header("Location:kullanici.php?i=hata");
            }
        }

        if(@$_GET["is"]=="sil"){
            $sil=$db->prepare("DELETE FROM  `kullanici` WHERE `id` = :i ");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <a href="kullanici_ekle.php" class="btn btn-primary" style="margin-right: 15px;">Kullanıcı Ekle +</a>
                            <?php
                                    if(@$_GET["i"]=="ekle"){
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <?php
                                            $cek=$db->prepare("SELECT * FROM `kullanici` ORDER BY `id` DESC");
                                            $cek->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td class="center">
                                                <?php if($row["aktif"] == 1){ ?>
                                                <a href="kullanici.php?is=aktif&id=<?=$row["id"]?>&drm=<?=$row ["aktif"] ?>"
                                            ...
                                            <td>
                                                ...
                                                <a href="kullanici.php?is=sil&id=<?=$row["id"]?>"
